/* Mancs & Móka — stabil, nem White Label verzió
   A fordítások külön lang/*.js fájlokban vannak, szinkron script tagekkel betöltve.
   Nincs fetch(), nincs config.js, nincs újratöltéses nyelvváltás. */
const TRANSLATIONS = window.MANCS_TRANSLATIONS || {};
...
(() => {
  ...
  function renderServices() {
    ...
    BUSINESS.services.forEach((s,i)=>{
      if(grid){
        ...
        a.querySelector("h3").textContent=local(s.name); a.querySelector("p").textContent=local(s.description); a.querySelector("b").textContent=s.price; a.querySelector("a").textContent=TRANSLATIONS[lang].service_book; grid.appendChild(a);
      }
  ...
  function renderPrices() {
    ...
    BUSINESS.prices.forEach(item=>{
      ...
      if(item.featured){const pop=document.createElement("span"); pop.className="popular"; pop.textContent=TRANSLATIONS[lang].popular; a.appendChild(pop);}
      ...
      const link=document.createElement("a"); link.href="#booking"; link.textContent=TRANSLATIONS[lang].choose; a.appendChild(link); grid.appendChild(a);
    });
  }

  function setLanguage(next) {
    if(!supported.includes(next)) return; lang=next; setStored(next);
    applyStaticTranslations(); applyBusiness(); renderServices(); renderPrices();
  }
  window.setLanguage=setLanguage;

  document.addEventListener("DOMContentLoaded",()=>{
    applyStaticTranslations(); applyBusiness(); renderGallery(); renderServices(); renderPrices();
